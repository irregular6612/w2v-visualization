...
def read_corpus_file(file_path, encoding='utf-8', sample_size=None):
    """코퍼스 파일을 읽어 라인 리스트로 반환합니다. 샘플링 옵션 제공."""
...
def analyze_corpus(ko_lines, en_lines, dataset_name):
    ...
    # 기본 통계
    print(f"분석 문장 쌍 수: {len(ko_lines)}")
...
def plot_statistics(train_stats, dev_stats, test_stats):
    ...
    plt.subplot(2, 2, 3)
    plt.scatter(train_stats['ko_token_counts'], train_stats['en_token_counts'], alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('korean_parallel_stats_light.png')
    plt.show()
...
def main():
    ...
    # 샘플 크기 설정 (처리 속도 개선을 위해)
    train_sample_size = 5000  # 학습 데이터에서 5000개 샘플링
    dev_sample_size = None    # 개발 데이터는 전체 사용 (작은 크기)
    test_sample_size = None   # 테스트 데이터는 전체 사용 (작은 크기)
    
    # 데이터 로드 (샘플링 적용)
    print("데이터 로드 중...")
    ko_train_lines = read_corpus_file(ko_train_file, sample_size=train_sample_size)
    en_train_lines = read_corpus_file(en_train_file, sample_size=train_sample_size)
    ko_dev_lines = read_corpus_file(ko_dev_file, sample_size=dev_sample_size)
    en_dev_lines = read_corpus_file(en_dev_file, sample_size=dev_sample_size)
    ko_test_lines = read_corpus_file(ko_test_file, sample_size=test_sample_size)
    en_test_lines = read_corpus_file(en_test_file, sample_size=test_sample_size)
    
    # 데이터 분석
    print("데이터 분석 중...")
    train_stats = analyze_corpus(ko_train_lines, en_train_lines, "학습 데이터 (샘플)")
    ...
    print("분석 완료! 'korean_parallel_stats_light.png' 파일에 시각화 결과가 저장되었습니다.")
